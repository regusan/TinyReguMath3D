...
// Rotate m by angle (radians) around axis LH
...
template <typename T> mat4<T> lookAt(const vec3<T>& eye, const vec3<T>& center, const vec3<T>& up) {
    ...
    result[1][1] = u.y;
    result[2][1] = u.z;
    result[0][2] = f.x; // no negation: +Z forward
    result[1][2] = f.y;
    result[2][2] = f.z;
